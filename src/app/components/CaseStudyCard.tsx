...
export interface CaseStudyCardProps {
  title: string;
  description: string;
  to: string; // internal route, e.g. "/design-ops/design-system-vr"
  headerImage?: string;
  pills: PillTag[];
  index?: number;
}
...
export function CaseStudyCard({
  ...
  return (
    <motion.div
      ...
      className="flex flex-col border border-[#e0e0e0] rounded-lg overflow-hidden bg-white hover:border-[#0BBA3F]/40 transition-colors hover:shadow-lg hover:shadow-[#0BBA3F]/5"
    >
      {/* Header image */}
      ...
      <div className="p-5 md:p-6 flex flex-col flex-1">
        ...
        <div className="mt-auto">
          <Link
            to={to}
            className="inline-flex items-center gap-2 font-['Roboto_Slab',serif] text-[13px] text-[#0BBA3F] bg-[rgba(11,186,63,0.12)] px-4 py-2 rounded hover:bg-[rgba(11,186,63,0.2)] transition-colors group"
          >
            View Case Study
            <FontAwesomeIcon
              icon={faArrowRight}
